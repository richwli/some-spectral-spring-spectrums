...
// optional vars
// NOTE: For decimation to work, decimationFactor MUST match zeroPaddingFactor
const int decimationFactor = 2; // no decimation = 0; force less resolution decimating values
const int zeroPaddingFactor = 2; // no padding = 0; force more resolution by end padding 0's
...
void writeFrame(float startingTime, int originalWindowSize, vector<double> samples, int sampleRate) { 
    ...
    while(frequencyBin <= antiAliasedNyquist) {
        ...
        // Idk if this works but only save values that is a multiple of decimation factor, so I think
        // this creates single pass decimation of data by saving every nth decimation_factor value
        if(iterationCounter % decimationResult == 0) {
            // normalize amplitudes by decimation to ensure inverse fft is same volume
...
int main() {
    ...
    vector<double> currentWindow(zeroPaddingResult*windowSizeParam,0);
    float startingTime;
    double hannWindow;
    ...
    while(endingIndex < totalSamples-1) {
        ...
        currentWindow.assign(zeroPaddingResult*windowSizeParam,0); // restore vector to og size and 0 for next iter
    }
