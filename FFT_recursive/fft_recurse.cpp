...
/*
complex<double> xn(int k,int sample) {
    complex<double> xk;
    for(int n=0; n < N; n++) {
...
*/
/*
complex<double> xn(int N,int k,int sample) {
    complex<double> xk;
    for(int n=0; n < N; n++){
        double bn_val = -1 * bn(n,k); // seems correct
        // cout<<"bn_val: "<<bn_val<<endl;
        // double hanWindow = pow(sin(3.14159 * n / N),2);
        // complex<double> hanWindowComplex(hanWindow); // hann window
        // int intSample = static_cast<int> (sampleC * maxAmplitude);
        complex<double> complexSample(sample);
...
/*
complex<double> fft(int k,int start, int end) {
    const int sample = samples[start];
    //cout<<start<<endl;
    if (start == end) {return sample;}
    // const complex<double> xn1 = xn(k,sample);
    const int midpoint = (end+start)/2;
    // cout<<start<<' '<<midpoint<<' '<<end<<endl;
    const complex<double> wk1 = wk(k);

    const complex<double> leftXn = fft(k,start,midpoint);
    const complex<double> rightXn = wk1 * fft(k,midpoint+1,end);
    // cout<<"wk1: "<<wk1<<" xn1: "<<xn1<<" total: "<<total<<endl;
    return leftXn + rightXn;
}
*/
vector<complex<double>> fft(vector<complex<double>> samples) {
    const int samplesLength = samples.size();
    const int n2 = samplesLength/2;
    if (samplesLength == 1) { 
        return samples;
    }
    vector<complex<double>> doubleResult(samplesLength);
    vector<complex<double>> evenVector(samplesLength/2);
    vector<complex<double>> oddVector(samplesLength/2);
    for(int n=0; n<samplesLength/2; n++) {
        evenVector[n] = samples[2*n];
        oddVector[n] = samples[2*n+1];
    }
    vector<complex<double>> fftEven = fft(evenVector);
    vector<complex<double>> fftOdd = fft(oddVector);
    for(int k=0; k<n2; k++) {
        // const complex<double> wk1 = wk(k);
        const complex<double> wk1 = polar(1.0,-2*3.14159*k/samplesLength);
        const complex<double> oddSide = wk1 * fftOdd[k];
        doubleResult[k] = fftEven[k] + oddSide;
        doubleResult[k+n2] = fftEven[k] - oddSide;
    }
    return doubleResult;
}
int main() {
    const string octaves[12] = {
        "C",
        "C#_Db",
        "D",
        "D#_Eb",
    ...
    // const int samplesLength = sizeof(samples)/sizeof(samples[0]);
    // double* evens = fEvens(samples);
    // cout<<sizeof(samples)/sizeof(samples[0])<<endl;
    
    //const complex<double> fk = fft(k,0,samplesLength-1);
    vector<complex<double>> pain = fft(samples);
    for (int k=0; k<samples.size(); k++) {
        complex<double> fk = pain[k];
        // NOTE: THIS IS MAGNITUDE OUTPUT
